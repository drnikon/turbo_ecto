defmodule Turbo.Ecto.Builder do
  ...
  when use `assoc`
      ...
      iex> Turbo.Ecto.Builder.run(Turbo.Ecto.Product, params)
      #Ecto.Query<from p0 in Turbo.Ecto.Product, left_join: c1 in assoc(p0, :category), where: like(c1.name, \"%elixir%\"), limit: 10, offset: 0>

  when use `and` && `or` && `assoc` condition

      iex> params = %{"q" => %{"category_name_or_name_and_body_like" => "elixir"}}
      iex> Turbo.Ecto.Builder.run(Turbo.Ecto.Product, params)
      #Ecto.Query<from p0 in Turbo.Ecto.Product, left_join: c1 in assoc(p0, :category), where: like(p0.body, \"%elixir%\") or (like(c1.name, \"%elixir%\") or like(p0.name, \"%elixir%\")), limit: 10, offset: 0>
